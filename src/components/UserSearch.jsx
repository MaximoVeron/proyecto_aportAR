...
const UserSearch = ({ onSelectUser }) => {
  ...
  const { getConversationWithUser, sendMessage, getContactableUsers } = useMessaging();
  ...
  const handleStartConversation = (user) => {
    ...
    if (existingConversation) {
      ...
      // Crear nueva conversación enviando un mensaje inicial
      const result = sendMessage(user.id, `¡Hola ${user.name}! Me gustaría conversar contigo.`);
      ...
      if (result.success) {
        ...
        setTimeout(() => {
          const newConversation = getConversationWithUser(user.id);
          if (newConversation) {
  ...
  return (
    <div className="space-y-4">
      <div className="relative">
        ...
        <Input
          type="text"
          placeholder="Buscar profesores, administrativos o administradores..."
          value={searchQuery}
      ...
      {searchQuery.trim() !== '' && searchResults.length === 0 && (
        <div className="text-center py-8 text-gray-500 dark:text-gray-400">
          ...
          <p className="text-sm mt-1">Solo puedes buscar profesores, administrativos y administradores</p>
        </div>
      )}
    </div>
  );
